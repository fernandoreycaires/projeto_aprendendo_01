<?php
    ...
    if ($_SESSION['cnpj_session'] and $_SESSION['user_session'] and $_SESSION['pass_session']){
        ...
        $atend_conv = isset($_POST['tCon'])?$_POST['tCon']:'';
        ...
        if($atend_cpf == "" and $atend_rg == "" and $atend_cn == ""){
            ...
            ?>
                ...
                <html lang="pt-br">
                    <body>
                        <form method="post" name="salvacli" id="salvacli" action="atendimento_abrir.php">
                            <input type="hidden" name="msg" value="<?php echo $checado;?>">
                            <input type="hidden" name="id" value="<?php echo $id_cal;?>">
                        </form>
                    </body>
        ...
        $paciente_sql = "SELECT id, nome, cel1 FROM clientes WHERE cpf != '' AND cpf = '".$atend_cpf."' OR rg != '' AND rg = '".$atend_rg."' OR certnasc != '' AND certnasc = '".$atend_cn."'";
        ...
        //VERIFICA SE AGENDAMENTO CONTEM LEAD, E FAZ TRATAMENTO PARA INSERIR DOCUMENTO NO CAD DE LEAD
        $calsql = "SELECT id_lead FROM agenda WHERE id_data = '".$id_cal."'";
            $calsql_query = $cx->query ($calsql);
            while ($callead = $calsql_query->fetch()){
                $cal_leadid = $callead['id_lead'];
            }
            if ($cal_leadid != ""){
                $cpflead = "SELECT cpf, rg, cn FROM lead WHERE id_lead = '".$cal_leadid."'";
                ...
                while ($lead_doc = $leaddoc_query->fetch()){
                    ...
                    $lead_cn = $lead_doc['cn'];
                                ...
                                //INSERE DOCUMENTOS PARA VINCULAR O LEAD COM UM CONTATO
                                    ...
                                    if($lead_cpf == ""){    
                                        if ($atend_cpf != ""){
                                            $leadcpfsql = "UPDATE lead SET cpf = '".$atend_cpf."' WHERE id_lead = '".$cal_leadid."'";
                                    ...
                                    if($lead_rg == ""){
                                        if ($atend_rg != ""){
                                            $leadrgsql = "UPDATE lead SET rg = '".$atend_rg."' WHERE id_lead = '".$cal_leadid."'";
                                    ...
                                    if($lead_cn == ""){
                                        if ($atend_cn != ""){
                                            $leadcnsql = "UPDATE lead SET cn = '".$atend_cn."' WHERE id_lead = '".$cal_leadid."'";
                                    ...
                                    $lead_contato = "INSERT INTO lead_contato (id_lead, cnpj, dia, hora, obs,agendado, retornar) VALUES ('".$cal_leadid."','".$cnpj."','".$atend_ini_data."','".$atend_ini_hora."','Abriu ficha de atendimento','S','N')";
        ...
        if ($confirm_ini != "$atend_ini_data" or $confirm_ini == ""){
            ...
            //PREPARANDO COMANDO SQL PARA SER ENVIADO PARA TABELA ATENDIMENTO
                $sql_cad = "INSERT INTO atendimento (nome , id_pac, id_lead ,".$documento." , cel, convenio , crm , doutor , cnpj , retorno , ini_data, ini_hora, obs) values ('{$atend_nome}' , '{$pac_id}', '{$cal_leadid}' ,'{$documentovar}' , '{$atend_cel}', '{$atend_conv}' ,'{$atend_crm}' , '{$atend_doc}' , '{$cnpj}' , '{$atend_ret}' , '{$atend_ini_data}', '{$atend_ini_hora}', '{$atend_obs}')";
                    ...
                    <div class="container-fluid theme-showcase estilo" role="main">
                            ...
                            <div class="row">
                                    <div class="col-md-12">
                                        ...
                                        <p align='center'><a href="home_cal.php" class="btn btn-sm btn-success">Entendi</a></p>
                                    </div>
                            </div>
                        <br>
                        <footer id="rodape">
