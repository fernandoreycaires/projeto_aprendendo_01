...
$orc_cortesia2= $_POST['tCort2'];
    ...
    $sql = "INSERT INTO mailing_obs (id_m ,obs , dia , hora, operador, id_op, ret_dia, ret_hora, retornar ) VALUES ('{$cadid_m}' , '{$cadobs}' , '{$data_atual}' ,'{$hora_atual}' ,'{$cadop}','{$cadidop}','{$cadretdata}','{$cadirethora}','{$cadret}')";
    // ENVIA COMANDO SQL INSERIDO NA VARIAVEL ACIMA PARA O BANCO
    $q = $cx->query ($sql);

    if ($q) {
        $msg = 'Negociação ou Observação cadastrada com sucesso !';
        ...
        if ($orc_prod != "" and $orc_negoc != ""){
                        ...
                        //PRIMEIRO FAZ UM SELECT PARA BUSCAR ID DA OBSERVAÇÃO QUE ACABOU DE SER CRIADA
                            $obssql = "SELECT LAST_INSERT_ID(id_mo) FROM mailing_obs WHERE id_m = '{$cadid_m}' ORDER BY id_mo DESC LIMIT 1";
                            $enviaobs = $cx->query ($obssql);
                                while ($obs = $enviaobs->fetch()){
                                    $obs_idmo = $obs['LAST_INSERT_ID(id_mo)'];
                                }

                        // CRIA COMANDO SQL PARA INSERÇÃO NA TABELA ORÇAMENTO (orc)
                            $orcsql = "INSERT INTO orc (modo_pag ,tipo_pag , dia_pag , dia, hora, contato, operador, id_op, id_mo, id_m ) VALUES "
                                    . "('{$orc_modopag}' , '{$orc_tipopag}' , '{$orc_pagdata}' , '{$data_atual}' ,'{$hora_atual}' ,'{$orc_cont}','{$cadop}','{$cadidop}','{$obs_idmo}','{$cadid_m}')";
                            $orc = $cx->query ($orcsql);
                            ...
                            if ($orc){
                                    ...
                                    $orcsql = "SELECT id_orc FROM orc WHERE id_mo = '{$obs_idmo}'";
            ...
            <html lang="pt-br">
                <body>
                    <form method="post" name="salvacli" id="salvacli" action="mailing_obs.php">
                        ...
                        <input type="hidden" name="id_m" value="<?php echo $cadid_m;?>">
                    </form>
                </body>
